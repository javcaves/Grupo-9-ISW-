...
export const obtenerUsuariosDelProyecto = async(idProyecto, filtros = {})=>{
    try{
        ...
        //filtros de la entidad de usuario
        if (filtros.rol){
...
export const asignarUsuarioAProyecto = async(idProyecto, data, ejecutor)=>{
    try{
        ...
        const existe = await ProyectoUsuarioRepository.find({
            where: {id_proyecto: parseInt(idProyecto), id_usuario: data.id_usuario, activo: true}
        });

        if (existe) throw new Error('el usuario ya está asignado a un proyecto');
        ...
        //para obtener usuario con sus datos en la respuesta
...
export const desvincularUsuarioDeProyecto = async(idProyecto, idUsuario, ejecutor)=>{
    try{
        const asignacion = await ProyectoUsuarioRepository.find({
            where: {id_proyecto: parseInt(idProyecto), id_usuario: idUsuario, activo: true}
        });

        if (!asignacion) throw new Error('no se encontró la asignacion del usuario en este proyecto');

        asignacion.activo = false;
        asignacion.fecha_termino = new Date();

        await ProyectoUsuarioRepository.save(asignacion);
        return [{message: 'usuario desvinculado con exito'}, null];
    }catch (error){
        return [null, error.message];
    }
};
